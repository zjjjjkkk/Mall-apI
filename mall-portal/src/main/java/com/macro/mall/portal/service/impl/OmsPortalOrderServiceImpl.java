...
/**
 * 前台订单管理Service
 * Created by zjjjjkkk on 2018/8/30.
 */
@Slf4j
@Service
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class) // 新增事务保证原子性
    public void cancelOrder(Long orderId) {
        ...
        if (CollectionUtils.isEmpty(cancelOrderList)) {
            log.info("订单{}无需取消：非未付款状态或已删除", orderId);
            return;
        }
        ...
        if (cancelOrder != null) {
            ...
            if (!CollectionUtils.isEmpty(orderItemList)) {
                for (OmsOrderItem orderItem : orderItemList) {
                    ...
                    Integer releaseQuantity = orderItem.getProductQuantity();

                    // 新增：查询当前锁定库存，避免盲扣
                    Integer currentLockStock = portalOrderDao.getLockStockBySkuId(skuId);
                    if (currentLockStock == null) {
                        log.error("订单{}取消失败：SKU{}不存在", orderId, skuId);
                        Asserts.fail("库存数据异常，无法释放！");
                    }

                    // 兼容场景：锁定库存已为0（重复释放/已释放），不抛异常
                    if (currentLockStock == 0) {
                        log.warn("订单{}取消时，SKU{}锁定库存已为0，无需释放", orderId, skuId);
                        continue;
                    }

                    // 锁定库存不足，抛异常（保留原有逻辑）
                    if (currentLockStock < releaseQuantity) {
                        log.error("订单{}取消失败：SKU{}锁定库存{}不足释放数量{}",
                                orderId, skuId, currentLockStock, releaseQuantity);
                        Asserts.fail("库存不足，无法释放！");
                    }

                    // 执行库存释放
                    int count = portalOrderDao.releaseStockBySkuId(skuId, releaseQuantity);
                    if (count == 0) {
                        log.warn("订单{}取消：SKU{}库存释放SQL执行无影响行数（锁定库存已为0）", orderId, skuId);
                        continue; // 不再抛异常，避免流程中断
                    }
                    log.info("订单{}取消：SKU{}释放锁定库存{}", orderId, skuId, releaseQuantity);
                }
            }
            ...
            if (cancelOrder.getUseIntegration() != null) {
                ...
                if (member != null) { // 新增空指针防护
                    memberService.updateIntegration(cancelOrder.getMemberId(), member.getIntegration() + cancelOrder.getUseIntegration());
                }
            }
            log.info("订单{}取消流程执行完成", orderId);
        }
    }
